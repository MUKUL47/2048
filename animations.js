class Animations {
  ...
  animateNodes(nodes, isLeft, isXAxis) {
    for (let i = 0; i < nodes.length; i++) {
      for (let j = 0; j < 4; j++) {
        ...
        const absoluteAxis = isXAxis
          ? Math.abs(nodes[i][j].posX - j)
          : Math.abs(nodes[i][j].posY - i);
        const direction = !isLeft
          ? absoluteAxis * 100 + absoluteAxis * 5
          : -absoluteAxis * 100 - absoluteAxis * 5;
        element.children[0]
          .animate(
            [
              { transform: `translate${(isXAxis && "X") || "Y"}(0px)` },
              {
                transform: `translate${
                  (isXAxis && "X") || "Y"
                }(${direction}px)`,
